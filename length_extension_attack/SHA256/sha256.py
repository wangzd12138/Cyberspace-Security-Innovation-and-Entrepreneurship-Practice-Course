...
def Lengthwith64bit(Length):
    ...
    for j in range(64):
        ...
        if (j + 1) % 8 == 0:
            asHex.append(asBin)
            asBin=""
    return [int(string,2) for string in asHex]
...
def ROTR(n,x):
    try:
        ...
        raise ValueError(f'n should be less than 32 in sha256 for RotateRight {n}()')
...
class Sha256:
    ...
    def __init__(self, message, originHash=None, salt=None):
        if message is not None and type(message) is not str:
            raise TypeError(f'{self.__class__.__name__}() argument 1 must be string, not {type(message).__name__}')
        ...
        tmp = ''.join(self.hash(parsed)[idx][2:] for idx in range(8))
        self.sha256 = tmp
    ...
    def hash(self, preprocessed):
        ...
        lastHash = messageBlocks[-1]
        asHex = [0 for _ in range(len(lastHash))]
