#include <stdlib.h>
#include <string.h>
#include<stdio.h>
#include "sm4.h"
#include<stdint.h>

#define  CROL(x, b) ((((x) & 0xFFFFFFFF) << (b)) | ((x) >> (0x20 - (b))))


#ifndef  GET_UINT32_BE
#define  GET_UINT32_BE(n, b, i)                       \
{                                                       \
    (n) = ( (uint32_t) (b)[(i)    ] << 24 )             \
        | ( (uint32_t) (b)[(i) + 1] << 16 )             \
        | ( (uint32_t) (b)[(i) + 2] <<  8 )             \
        | ( (uint32_t) (b)[(i) + 3]       );            \
}
#endif


#ifndef  PUT_UINT32_BE
#define  PUT_UINT32_BE(n, b ,i)                       \
{                                                       \
    (b)[(i)    ] = (unsigned char) ( (n) >> 24 );       \
    (b)[(i) + 1] = (unsigned char) ( (n) >> 16 );       \
    (b)[(i) + 2] = (unsigned char) ( (n) >>  8 );       \
    (b)[(i) + 3] = (unsigned char) ( (n)       );       \
}
#endif


#define  SM4_NON_LINEAR_OP(in, out) \
{                                                           \
	(out)  = (  SM4_SBOX[ ((in) >> 24) & 0x0FF ] ) << 24; \
	(out) |= (  SM4_SBOX[ ((in) >> 16) & 0x0FF ] ) << 16; \
	(out) |= (  SM4_SBOX[ ((in) >> 8)  & 0x0FF ] ) << 8;  \
	(out) |= (  SM4_SBOX[ (in)         & 0x0FF ] );       \
}


unsigned int  SM4_CK[32] = {
	0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
	0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
	0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
	0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};


unsigned char  SM4_SBOX[256] = {
	
};


unsigned int  SM4_FK[4] = { 0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC };

static void sm4_key_schedule(const unsigned char* key, unsigned int* rk)
{
	unsigned int tmp1, tmp2, K[36];
	int i;

	 GET_UINT32_BE(K[0], key, 0);
	 GET_UINT32_BE(K[1], key, 4);
	 GET_UINT32_BE(K[2], key, 8);
	 GET_UINT32_BE(K[3], key, 12);

	K[0] ^=  SM4_FK[0];
	K[1] ^=  SM4_FK[1];
	K[2] ^=  SM4_FK[2];
	K[3] ^=  SM4_FK[3];

	
	for (i = 0; i < 32; i++)
	{
		
		tmp1 = K[i + 1] ^ K[i + 2] ^ K[i + 3] ^  SM4_CK[i];

		
		 SM4_NON_LINEAR_OP(tmp1, tmp2);

		
		rk[i] = K[i + 4] = K[i] ^ (tmp2 ^  CROL(tmp2, 13) ^  CROL(tmp2, 23));
	}
}

void   sm4_crypt(const unsigned char* key, int forEncryption, const unsigned char* in, unsigned char* out)
{
	unsigned int rk[32], X[36], tmp1, tmp2;
	int i;

	sm4_key_schedule(key, rk);

	 GET_UINT32_BE(X[0], in, 0);
	 GET_UINT32_BE(X[1], in, 4);
	 GET_UINT32_BE(X[2], in, 8);
	 GET_UINT32_BE(X[3], in, 12);

	for (i = 0; i < 32; i++)
	{
		tmp1 = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[(forEncryption ? i : 31 - i)];

		 SM4_NON_LINEAR_OP(tmp1, tmp2);

		unsigned int tt = (tmp2 ^  CROL(tmp2, 2) ^  CROL(tmp2, 10) ^  CROL(tmp2, 18) ^  CROL(tmp2, 24));

		X[i + 4] = X[i] ^ tt;
	}
	 PUT_UINT32_BE(X[35], out, 0);
	 PUT_UINT32_BE(X[34], out, 4);
	 PUT_UINT32_BE(X[33], out, 8);
	 PUT_UINT32_BE(X[32], out, 12);
}
